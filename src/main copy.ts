...
} from "./component/series/svg/basic-line-series";
import { LabelSeries } from "./component/series/svg/label-series";
import {
  BasicBoxplotSeries,
  BoxplotModel,
} from "./component/series/svg/basic-boxplot-series";
...
} from "./component/series/svg/stacked-vertical-bar-series";
import {
  GroupedVerticalBarSeries,
  GroupedVerticalBarSeriesConfiguration,
} from "./component/series/svg/grouped-vertical-bar-series";
...
} from "./component/series/svg/basic-area-series";
import {
  BasicCanvasScatterPlotModel,
  BasicCanvasScatterPlot,
} from "./component/series/canvas/basic-canvas-scatter-plot";
...
import {
  ...
  TopologyData,
} from "./component/series/svg/basic-topology";
...
import {
  ...
  ScaleType,
  Direction,
} from "./component/chart/chart-configuration";

import { lineData } from "./component/mock-data/line-one-field-data";
import {
  BasicCanvasLineSeries,
  BasicCanvasLineSeriesConfiguration,
} from "./component/series/canvas/basic-canvas-line-series";
...
} from "./component/series/canvas/basic-canvas-trace";
...
import {
  MiChart,
  OptionConfiguration,
  MiccBaseConfiguration,
} from "./component/mi-chart";
...
const simpleWebglLineSeriesExample = () => {
  ...
  (select("#json-configuration").node() as any).innerHTML = JSON.stringify(
    commonConfiguration,
    null,
    "\t"
  );
...
const simpleSvgStackedColumnSeriesExample = () => {
  ...
  chart = SvgStackedBarChart(
    commonConfiguration,
    stackedVerticalColumnSeries
  ).draw();
  chart.chartItemEvent.subscribe((item: ChartItemEvent) => {
...
const webGLBigDataLineSeriesSample = () => {
  ...
  const parseData = () => {
    ...
    for (let i = 0; i < tracePoints.length; i++) {
      ...
      // test data 늘리기
      const tempRow: BasicCanvasWebglLineSeriesOneModel =
      ...
      const configuration: BasicCanvasWebglLineSeriesOneConfiguration = {
        selector:
          (seriesColor === "#EA3010" ? "webgl-trace-alarm" : "webgl-trace") + i,
        xField: "x",
        yField: "y",
        dot: {
          radius: 4,
  ...
  chart = WebglTraceChart(
    commonConfiguration,
    seriesList.concat(alarmSeriesList),
    optionList
  ).draw();
};
  ...
  const parseData = () => {
    ...
    for (let i = 0; i < tracePoints.length; i++) {
      ...
      const configuration: BasicCanvasTraceConfiguration = {
        selector:
          (seriesColor === "#EA3010" ? "canvas-trace-alarm" : "canvas-trace") +
          i,
        xField: "x",
        yField: "y",
        dot: {
...
const canvasTraceChart = () => {
  ...
  const traceData = range(numberPoints).map((d: number) => {
    ...
    // const i = index;
    const i =
      (d % numberPoints) / 3 === 0 ? 0 : parseFloat(randomY().toFixed(2));
...
const topologyExcute = () => {
  const groups = topologyData.result.frameworks.map((item: any) => {
    ...
    return new TopologyGroupElement(
      item.fullName,
      0,
      0,
      50,
...
const boxplot = () => {
  ...
  for (const [key, groupCount] of Object.entries(groupCounts)) {
    const tempCounts: string[] = (groupCount as number[]).map(
      (num) => num + ""
    );
...
const stackedBar = () => {
  csv(
    "./component/mock-data/age-groups.csv",
    (d: any, index: number, columns: string[]) => {
...
const groupedBar = () => {
  csv(
    "./component/mock-data/grouped-bar-data.csv",
    (d: any, index: number, columns: string[]) => {
      for (let i = 1, t = 0; i < columns.length; ++i) {
        t += d[columns[i]] = +d[columns[i]];
        d.total = t;
      ...
      const groupedBarChart = new BasicChart({
        ...
        axes: [
          ...
          },
          {
            ...
            max: max(data, (d) =>
              max(result.columns.slice(1), (key) => d[key])
            ),
          },
        ],
...
const areaChart = () => {
  const parseTime = timeParse("%d-%b-%y");
  csv(
    "./component/mock-data/area-data.csv",
    (d: any, index: number, columns: string[]) => {
